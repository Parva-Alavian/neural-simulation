...
def spectrogram( t:np.array, res: list[ModelBase],min_fq=1, max_fq=100, smoothing = None, t_start = 2, t_end=None, **kwargs):
...
def fq_curve(I,circuit_params,params_set="ExtendedV1.json",sample_size=30,save_dir=  None):
    ...
    for ind,i in enumerate(I):    
        ...
        for j in range(sample_size):
            print(j)
            t, res = run(30, changes = experiment , dt=0.0001, path=None, params_set=params_data)
...
I = np.linspace(0.3,0.5,11)
sample_size = 20

experiment_name = "first_half"
save_dir = "./freq_curves/closing_loop3/" + experiment_name
